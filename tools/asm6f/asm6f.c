...
/* Locally modified for Visual Studio 2026 C17 compatibility. */
...
#define fatal_error(...) { fprintf(stderr, "\nError: "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n\n"); DIE(); }
#define message(...) if (verbose) { printf(__VA_ARGS__); }
...
int getvalue(char **str) {
	...
	if(*s=='$') {   //hex---------------------
		s++;
		if(!*s) {
			ret=(int)addr;//$ by itself is the PC
		} else do {
hexi:	   j=hexify(*s);
			s++;
			chars++;
...
void addcomment(char* text) {
	...
	if(lastcommentpos == filepos) {
		...
		char* oldtext = c->text;
		size_t oldtextlen = strlen(oldtext);
...
void pad(label *id, char **next) {
	...
	if(addr<0) {
		...
		dependant=0;
		count=(int)(eval(next,WHOLEEXP)-addr);
		filler(count,next);
	}
}
