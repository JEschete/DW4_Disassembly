...
| `$12` | Battle setup and combat services | Verified | 50-entry directory `$8000-$8063`; seven-entry nested dispatch table `$8493`; pointer-selected setup maps and records `$8A36-$8EAC`; further dispatch tables at `$A541/$A9ED/$AACA/$B67A`; `$8064` initializes battle globals. Selector-`$x3` BRK handlers for `$D3/$E3/$F3` live here. Directory entries `$2F-$31` address the script-like data at `$BDC9-$BF67`, whose interpreter is not yet identified. |
| `$13` | Battle presentation and state routing | Verified | 28-entry mixed directory `$8000-$8037`; bounded lookup arrays at `$8AEF/$8B24`, presentation lookups `$B80B/$B967`, parallel tables `$BA4E-$BBCA`, and seven command streams `$BBCB-$BFA6` route battle state and presentation. |
